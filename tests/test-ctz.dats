...
static int
brute_force_ctz_uintmax (uintmax_t u)
{
  assert (u != 0);
  int count = 0;
  while ((u & 1) == 0)
...
#define my_lshift(x, i) ((x) << (i))
#define my_isgtz(x) ((0 < (x)) ? atsbool_true : atsbool_false)

%}

macdef brute_force_ctz (u) =
  $extfcall (int, "brute_force_ctz_uintmax", ,(u))

implement
g1uint_isgtz<usintknd> (x) =
  let
    extern fn
    my_isgtz {x : int} (x : usint x) :<> bool (0 < x) =
      "mac#my_isgtz"
  in
    my_isgtz (x)
  end
...
implement
g1uint_isgtz<uintptrknd> (x) =
  let
    extern fn
    my_isgtz {x : int} (x : uintptr x) :<> bool (0 < x) =
      "mac#my_isgtz"
  in
    my_isgtz (x)
  end
...
implement
g1uint_isgtz<uint8knd> (x) =
  let
    extern fn
    my_isgtz {x : int} (x : uint8 x) :<> bool (0 < x) =
      "mac#my_isgtz"
  in
    my_isgtz (x)
  end
...
implement
g1uint_isgtz<uint16knd> (x) =
  let
    extern fn
    my_isgtz {x : int} (x : uint16 x) :<> bool (0 < x) =
      "mac#my_isgtz"
  in
    my_isgtz (x)
  end
...
implement
g1uint_isgtz<uint32knd> (x) =
  let
    extern fn
    my_isgtz {x : int} (x : uint32 x) :<> bool (0 < x) =
      "mac#my_isgtz"
  in
    my_isgtz (x)
  end
...
implement
g1uint_isgtz<uint64knd> (x) =
  let
    extern fn
    my_isgtz {x : int} (x : uint64 x) :<> bool (0 < x) =
      "mac#my_isgtz"
  in
    my_isgtz (x)
  end
...
fn {tk : tkind}
test_ctz_one_number (u : g0uint (tk)) :<> bool =
  let
    //val _ = $effmask_all $extfcall (int, "printf", "0x%llX\n",
    //                                $UNSAFE.cast{ullint} u)

    val u1 = g1ofg0 u
    val _ = $effmask_exn assertloc (isgtz u1)

    val b1 = brute_force_ctz (u)
    val (_ | b2) = ctz<tk> (u1)
    val b3 = ctz<tk> (u)
  in
    ((g1ofg0 b1) = b2) && (b1 = b3)
...
fn {tk : tkind}
test_ctz (u : g0uint (tk)) :<> bool =
  let
    stadef n = 8 * sizeof (g0uint (tk))
    val n = (i2sz 8) * sizeof<g0uint (tk)>
    ...
    loop {i : int | 0 <= i; i <= n} .<n - i>.
      ...
      if i = sz2i n then
        ...
        let
          extern fn
          my_lshift : (g0uint (tk), int) -<> g0uint (tk) =
            "mac#my_lshift"
          val u1 = my_lshift (u, i)
        in
          if iseqz u1 then
            loop {i + 1} (succ i)
          else if test_ctz_one_number (u1) then
            loop {i + 1} (succ i)
          else
            false
        end
...
main0 () =
  {
    ...
    val _ =
      for (i := 0; i < 100; i := succ i)
        assertloc (test_ctz<usintknd> ($UNSAFE.cast{usint} (test_numbers[i])))
    val _ =
      for (i := 0; i < 100; i := succ i)
        assertloc (test_ctz<uintknd> ($UNSAFE.cast{uint} (test_numbers[i])))
    val _ =
      for (i := 0; i < 100; i := succ i)
        assertloc (test_ctz<ulintknd> ($UNSAFE.cast{ulint} (test_numbers[i])))
    val _ =
      for (i := 0; i < 100; i := succ i)
        assertloc (test_ctz<ullintknd> ($UNSAFE.cast{ullint} (test_numbers[i])))
    val _ =
      for (i := 0; i < 100; i := succ i)
        assertloc (test_ctz<sizeknd> ($UNSAFE.cast{size_t} (test_numbers[i])))
    val _ =
      for (i := 0; i < 100; i := succ i)
        assertloc (test_ctz<uintptrknd> ($UNSAFE.cast{uintptr} (test_numbers[i])))
    val _ =
      for (i := 0; i < 100; i := succ i)
        assertloc (test_ctz<uint8knd> ($UNSAFE.cast{uint8} (test_numbers[i])))
    val _ =
      for (i := 0; i < 100; i := succ i)
        assertloc (test_ctz<uint16knd> ($UNSAFE.cast{uint16} (test_numbers[i])))
    val _ =
      for (i := 0; i < 100; i := succ i)
        assertloc (test_ctz<uint32knd> ($UNSAFE.cast{uint32} (test_numbers[i])))
    val _ =
      for (i := 0; i < 100; i := succ i)
        assertloc (test_ctz<uint64knd> ($UNSAFE.cast{uint64} (test_numbers[i])))
  }
