...
(********************************************************************)
(*
  Some of the following are not especially "safe".
*)

(* Convert an array to (initialized) bytes. *)
praxi {t : vt@ype}
array2bytes :
  {n : int}
  {p : addr}
...
(* Convert (initialized) bytes to an array. *)
praxi {t : vt@ype}
bytes2array :
  {n : int}
  {p : addr}
...
(* Convert an array to (uninitialized) bytes. *)
praxi {t : vt@ype}
array2bytesqmark :
  {n : int}
  {p : addr}
...
(* Convert (uninitialized) bytes to an array. *)
praxi {t : vt@ype}
bytesqmark2array :
  {n : int}
  {p : addr}
