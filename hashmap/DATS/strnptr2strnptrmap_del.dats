...
implement
strnptr2strnptrmap_del_strnptr (map, key) =
  let
    implement
    hashmap$value_vt_free<value_vt> (v) =
      strnptr_free v
  in
    strnptrmap_del<value_vt> (map, key)
  end

implement
strnptr2strnptrmap_del_strptr (map, key) =
  let
    val s = $UNSAFE.castvwtp1{key_vt} key
    prval _ = lemma_strnptr_param s
    val result = strnptr2strnptrmap_del_strnptr (map, s)
...
implement
strnptr2strnptrmap_del_string (map, key) =
