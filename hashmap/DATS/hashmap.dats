...
(*
  ...
  (indeed, better than more tedious alternatives), as long as you use
  the "entry" before you use the "exit".

  FIXME: But is there a *nice* way to *ensure* the entry and exit are
  used in the correct order?
  
  (On the other hand, to see it by inspection is likely to be easy.
  If the exit *does* occur before the entry, then you may have two
  copies of one view at the same time, thus violating linearity.)
*)
extern praxi
make_view_tunnel :
  {v : view}
...
hashmap_set {size} (map, key, value) =
  ...
  | @ map_vt_root root =>
    let
      ...
      val _ = root :=
        set_entry<hash_vt><key_vt, value_vt>
          {size} (size, tree, hash, key, value)
...
fn {key_vt, value_vt : vt@ype}
node_path_del
        (path : node_path_vt (key_vt, value_vt),
         key  : !RD(key_vt) >> _) :
    @{
      path = node_path_vt (key_vt, value_vt),
      entry_removed = bool
  ...
  let
    ...
    eliminate_array
            ...
            {mask           : int}
            {index          : int}
            (array          : node_path_array_vt (key_vt, value_vt,
                                                  population_map,
                                                  2, p_array,
                                                  mask, index)) :
        kv_t =
      let
        ...
        prval _ = lemma_g1uint_param index
      in
        if index = i2sz 1 then
          let
            val p_remaining_entry = p_array

            prval pf_right =
              array_v_unnil_nil
                {t, t?!}
                {p_array + index * sizeof (t) + sizeof (t?!)}
                pf_right

            prval @(pf_left1, pf_left2) =
              array_v_uncons {t} {p_array} {1} pf_left
            prval pf_left2 =
              array_v_unnil_nil 
                {t, t?!} {p_array + sizeof (t)}
                pf_left2
            val node = ptr_get<t> (pf_left1 | p_remaining_entry)
            prval pf_left =
              array_v_cons {t?!} {p_array} {0} (pf_left1, pf_left2)
            ...
            val- ~ node_vt_key_value key_value = node
          in
            key_value
          end
        else
          let
            ...
            val p_remaining_entry = ptr_succ<t> (p_array)

            prval pf_left =
              array_v_unnil_nil {t, t?!} {p_array} pf_left
            ...
            prval pf_right2 =
              array_v_unnil_nil
                {t, t?!}
                {p_array + sizeof (t?!) + sizeof (t)}
                pf_right2
            val node = ptr_get<t> (pf_right1 | p_remaining_entry)
            prval pf_right = array_v_cons (pf_right1, pf_right2)
    ...
    shrink_array
      ...
      let
        ...
        (* FIXME: Prove this. *)
        prval _ = $UN.prop_assert {old_popcount == length} ()
        val _ = assertloc (new_popcount = pred old_popcount)
    ...
    shorten_path (path : np_t) : np_t =
      ...
      | node_path_vt_key_value
        ...
        let
          ...
          (* FIXME: Prove this. *)
          prval _ = $UN.prop_assert {length == 1} ()

          prval _ = lemma_g1uint_param index
          prval pf_left = array_v_unnil_nil pf_left
          prval pf_right = array_v_unnil_nil pf_right
          prval pf_array =
            array_v_merge_entry
              {t?} {p_array} {length} {index}
              (pf_left, pf_entry, pf_right)
          val _ = array_ptr_free (pf_array, pf_mfree | p_array)
        in
          shorten_path
    ...
    fn
    del (path : node_path_vt (key_vt, value_vt),
         key  : !RD(key_vt) >> _) :
        @{
          path = node_path_vt (key_vt, value_vt),
          entry_removed = bool
      ...
      | ~ node_path_vt_list (path1, lst) =>
        let
          ...
          var lst_var : List_vt (kv_t) = lst
          var removed : bool
          val _ = remove_list_entry (lst_var, removed, key)
          val lst = lst_var
        in
          if removed then
            begin
  ...
  in
    if size = i2sz 0 then
      ...
      | @ map_vt_root root =>
        let
          val path =
            tree_to_node_path<hash_vt><key_vt, value_vt>
              (root.tree, key)
