...
implement
strnptr2strnptrmap_get_opt_strnptr (map, key) =
  let
    implement
    hashmap$value_vt_copy<value_vt> (v) =
      strnptr_copy v
  in
    strnptrmap_get_opt<value_vt> (map, key)
  end

implement
strnptr2strnptrmap_get_opt_strptr (map, key) =
  let
    val s = $UNSAFE.castvwtp1{key_vt} key
    prval _ = lemma_strnptr_param s
    val result = strnptr2strnptrmap_get_opt_strnptr (map, s)
...
implement
strnptr2strnptrmap_get_opt_string (map, key) =
