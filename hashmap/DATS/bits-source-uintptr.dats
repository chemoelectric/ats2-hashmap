...
extern fn
rshift_uintptr {u, i : int}
               (u    : uintptr u,
                i    : uint i) :<>
    [v : int] uintptr v = "mac#ats2_hashmap_bits_source_rshift"
...
infix >>
overload >> with rshift_uintptr of 1000
prefix ~
...
extern castfn
uintptr_of_uint : {u : int} uint u -<> uintptr u

extern castfn
uint_of_uintptr : {u : int} uintptr u -<> uint u
...
implement
make_bits_source_uintptr {num_bits} (num_bits) =
  let
    val mask = make_mask (num_bits)
    val bitsize = 8U * (sz2u sizeof<uintptr>)
    val q = g1uint_div (bitsize, num_bits)
    val r = g1uint_mod (bitsize, num_bits)
      ...
      in
        if i < index_bound then
          let
            ...
            val [bits : int] bits = (uint_of_uintptr u_shifted) & mask
...
implement
free_bits_source_uintptr bits_source =
